...
/* strcasestr is a GNU extension not available in newlib/devkitARM.
   This simple replacement scans for needle case-insensitively. */
    ...
    while (*p) {
        if (*p == '<') {
            ...
            if (!closing && (!strcmp(name, "script") || !strcmp(name, "style"))) {
                ...
                const char *end = ci_strstr(p, close_tag);
                if (end) {
            ...
            if (!closing && !strcmp(name, "title")) {
                ...
                if (gt) {
                    ...
                    const char *tend = ci_strstr(tstart, "</title");
                    if (tend && !out->title) {
            ...
            if (!closing && !strcmp(name, "a")) {
                const char *gt = strchr(p, '>');
                if (gt) {
                    const char *href_attr = ci_strstr(p, "href=");
                    if (href_attr && href_attr < gt) {
                        ...
                        if (quote && vend && vend < gt) {
                            ...
                            const char *tstart = gt + 1;
                            const char *tend = ci_strstr(tstart, "</a");
